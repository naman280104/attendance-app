...
class StudentProfileService{
  ...
  static Future<Map<String,dynamic>> getProfileDetials(context) async {
    ...
    if(token==null){
      ...
      try{
        var response = await http.get(Uri(scheme:'http', host: hostIP ,port: hostPort ,path: '/student/profile'), headers: {'Authorization': token});
  ...
  static Future<String> updateProfileDetials(Map<String,dynamic> profileDetails,context) async {
    ...
    if(token==null){
      ...
      // logic for fetch profile details
      try{
        var response = await http.put(Uri(scheme:'http', host: hostIP ,port: hostPort ,path: '/student/profile/update'), headers: {'Authorization': token},body: profileDetails);
          var responseBody = json.decode(response.body);
        if(response.statusCode == 200){
          print(responseBody);
           try{
